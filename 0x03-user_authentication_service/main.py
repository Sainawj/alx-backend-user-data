...
def reset_password_token(email: str) -> str:
    ...
    url = "{}/reset_password".format(BASE_URL)  # Endpoint for password reset request
    body = {'email': email}
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    url = "{}/reset_password".format(BASE_URL)  # Endpoint for updating password
    body = {
        'email': email,
...
if __name__ == "__main__":
    ...
    log_in_wrong_password(EMAIL, NEW_PASSWD)      # Test login with wrong password
    profile_unlogged()                            # Test profile access while logged out
    session_id = log_in(EMAIL, PASSWD)            # Log in and get session ID
    profile_logged(session_id)                    # Access profile while logged in
    log_out(session_id)                           # Log out
    reset_token = reset_password_token(EMAIL)     # Request a password reset token
